...
client.on('ready', () => {
    client.channels.get("520985093742002205").join(); 
    });
  ...
  **تم فك الباند عن الجميع بواسطة** : <@${message.author.id}>`) 
  ...
  if(command == 'bc') {
    if(message.member.hasPermission('ADMINISTRATOR')) {
      if(args.join(' ')) {
        message.channel.send(`Sending to all members...`).then(async m => {
          message.guild.members.forEach(async member => {

            member.send(args.join(' ')).catch(err => {
              if(err) console.log(`Couldn't send message to this member, ${member.user.tag}`)
            });
          });
        });
      } else return message.reply('!bc Message');
    } else return;
  }
});
